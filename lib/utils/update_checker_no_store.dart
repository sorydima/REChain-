...
class UpdateCheckerNoStore {
  ...
  Uri downloadUri(UpdateMetadata metadata) {
    // platform specific
    if (PlatformInfos.isWindows) {
      return Uri.parse('https://github.com/sorydima/REChain/-'
          '/jobs/artifacts/$metadata/raw/'
          'build/windows/runner/Release/rechainonline.msix?job=build_windows');
    } else {
  ...
  Future<void> launchUpdater(UpdateMetadata metadata) async {
    ...
    try {
      ...
      if (PlatformInfos.isWindows) {
        ...
        if (response.statusCode == 200) {
          final file = File('${dir.path}/rechainonline.msix');
