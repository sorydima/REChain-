...
[ -n "${RECHAINONLINE_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${RECHAINONLINE_ORIG_GROUP}.app/group.${RECHAINONLINE_NEW_GROUP}.app/g" "ios/REChain.Online Share/REChain.Online Share.entitlements"
	sed -i "" "s/group.${RECHAINONLINE_ORIG_GROUP}.app/group.${RECHAINONLINE_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${RECHAINONLINE_ORIG_GROUP}.app/group.${RECHAINONLINE_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${RECHAINONLINE_ORIG_GROUP}.app/${RECHAINONLINE_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${RECHAINONLINE_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${RECHAINONLINE_ORIG_TEAM}/${RECHAINONLINE_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for rechainonline.app and rechainonline.app.REChainOnline-Share)
...
[ -n "${RECHAINONLINE_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/rechainonline.ipa"
  rm -rf "${TMPDIR}"
}
