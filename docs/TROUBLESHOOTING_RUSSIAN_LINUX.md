...
sudo dpkg -i --force-depends rechainonline-4.1.7-amd64.deb
```

#### RPM пакеты (РЕД ОС, ОС «Альт», РОСА)
```bash
# Ошибка: "Failed dependencies"
sudo dnf install rechainonline-4.1.7-1.x86_64.rpm --skip-broken
sudo yum localinstall rechainonline-4.1.7-1.x86_64.rpm --nogpgcheck
...
*Руководство по устранению неполадок версии 4.1.7+1150*
